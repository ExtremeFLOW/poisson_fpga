...
__kernel void cg(__global float * restrict x, 
                 ...
                 __global float * restrict g1,
                 __global float * restrict g2,
                 __global float * restrict g3,
                 __global float * restrict g4,
                 __global float * restrict g5,
                 __global float * restrict g6,
                 __global float * restrict dx,
                 __global float * restrict dxt,
...
{   
    ...
    for(int i = 0; i < nb; i++){
        ...
        for(int j = 1; j < blk_len; j++){
            idx[j] = gd[k+j] - 1;
            float cur = rtr_copies3[M2-1] + w[idx[j]];
            #pragma unroll
