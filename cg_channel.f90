...
module cg_banked_fpga
  ...
  type, public, extends(ksp_t) :: cg_banked_fpga_t
     ...
     integer(c_intptr_t) :: cmd_queue, cl_cg_kernel, context,&
                            cl_gs_add_kernel, cmd_queue_blk,&
                            cl_gs_blk_kernel, cmd_queue_gs
...
contains
  ...
  subroutine cg_banked_fpga_init_device(this,idevice, iplatform, bitstream, lx)
    ...
    this%cmd_queue_gs = clCreateCommandQueue(this%context, device_ids(idevice), 0_8, err)
    this%cmd_queue_blk = clCreateCommandQueue(this%context, device_ids(idevice), 0_8, err)
    ...
    enddo
    c_kernel_name(irec+1)=C_NULL_CHAR
    this%cl_gs_add_kernel=clCreateKernel(prog,C_LOC(c_kernel_name),err)
    if (err.ne.0) stop 'clCreateKernel'
    print *, 'hmm' 
    kernel_name = "gs_blk"
    irec=len(trim(kernel_name))
    ...
    enddo
    c_kernel_name(irec+1)=C_NULL_CHAR
    this%cl_gs_blk_kernel=clCreateKernel(prog,C_LOC(c_kernel_name),err)
    if (err.ne.0) stop 'clCreateKernel'
    ...
    if (err.ne.0) stop 'clSetKernelArg'
    do i = 1,this%nbanks
       call cl_set_kernel_arg(this%cl_gs_add_kernel,this%cl_w(i), i-1) 
    end do
    do i = 1,this%nbanks
       call cl_set_kernel_arg(this%cl_gs_blk_kernel,this%cl_w(i), i-1) 
    end do
  ...
  subroutine cg_banked_fpga_populate(this, r, w, x, Xh,  c_Xh, bclst, gs_Xh, rtz1, beta)
    ...
    err=clSetKernelArg(this%cl_gs_add_kernel,4,sizeof(this%gs_m),C_LOC(this%gs_m))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_gs_add_kernel,5,sizeof(this%lo),C_LOC(this%lo))
    if (err.ne.0) stop 'clSetKernelArg'
    
    err=clSetKernelArg(this%cl_gs_blk_kernel,4,sizeof(this%nb),C_LOC(this%nb))
    if (err.ne.0) stop 'clSetKernelArg'
  ...
  subroutine host_kernel(this, gs_h, blst, n)
    ...
    !if (err.ne.0) stop 'clEnqueueReadBuffer'
    err=clFinish(this%cmd_queue)
    err=clFinish(this%cmd_queue_gs)
    err=clFinish(this%cmd_queue_blk)
    rnorm = sqrt(rtz1)
    print *, rnorm, rtz1
