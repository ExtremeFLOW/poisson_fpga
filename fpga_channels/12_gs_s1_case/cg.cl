...
{  
    ...
    for(int i = 0; i < nb; i++){
        ...
        for(int j =0; j < blk_len; j++){
            ...
            double value;
            switch(blk_data.bank[j]){
                case 0: value = w_1[blk_data.idx[j]]; break;
                case 1: value = w_2[blk_data.idx[j]]; break;
                case 2: value = w_3[blk_data.idx[j]]; break;
                case 3: value = w_4[blk_data.idx[j]]; break;
            }
            double cur = rtr_copies3[M2-1] + value;
            #pragma unroll
...
__kernel void gs_blk(__global double * restrict w_1,
    ...
    for(int i = 0; i < nb; i++){
        ...
        for(int j = 0; j < gs_blk_info.blk_len; j++){
            switch(gs_blk_info.bank[j]){
                case 0: w_1[gs_blk_info.idx[j]] = val; break;
                case 1: w_2[gs_blk_info.idx[j]] = val; break;
                case 2: w_3[gs_blk_info.idx[j]] = val; break;
                case 3: w_4[gs_blk_info.idx[j]] = val; break;
            }
        }
    }
