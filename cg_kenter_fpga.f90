...
module cg_fpga
  ...
  type, public, extends(ksp_t) :: cg_fpga_t
     ...
     integer(c_intptr_t) :: cmd_queue, cl_cg_kernel, cl_cg_gs_kernel,context
     integer(c_intptr_t) :: cl_x, cl_w, cl_p, cl_res
     integer(c_intptr_t) :: cl_g1, cl_g2, cl_g3, cl_g4, cl_g5, cl_g6, cl_w_tmp
...
contains
  ...
  subroutine cg_fpga_init_device(this,idevice, iplatform, bitstream, lx)
    ...
    enddo
    c_kernel_name(irec+1)=C_NULL_CHAR
    this%cl_cg_gs_kernel=clCreateKernel(prog,C_LOC(c_kernel_name),err)
    if (err.ne.0) stop 'clCreateKernel'
    ...
    this%cl_w = clCreateBuffer(this%context,ior(CL_MEM_READ_WRITE, CL_CHANNEL_3_INTELFPGA),&
                          this%array_size,C_NULL_PTR, err)
    if (err.ne.0) stop 'clCreateBuffer'
    this%cl_w_tmp = clCreateBuffer(this%context,ior(CL_MEM_READ_WRITE, CL_CHANNEL_2_INTELFPGA),&
                          this%array_size,C_NULL_PTR, err)
    if (err.ne.0) stop 'clCreateBuffer'
    ...
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,4,sizeof(this%cl_w),C_LOC(this%cl_w_tmp))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,5,sizeof(this%cl_mult),C_LOC(this%cl_mult))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,6,sizeof(this%cl_g1),C_LOC(this%cl_g1))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,7,sizeof(this%cl_g2),C_LOC(this%cl_g2))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,8,sizeof(this%cl_g3),C_LOC(this%cl_g3))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,9,sizeof(this%cl_g4),C_LOC(this%cl_g4))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,10,sizeof(this%cl_g5),C_LOC(this%cl_g5))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,11,sizeof(this%cl_g6),C_LOC(this%cl_g6))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,12,sizeof(this%cl_dx),C_LOC(this%cl_dx))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,13,sizeof(this%cl_dxt),C_LOC(this%cl_dxt))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,14,sizeof(this%cl_mask),C_LOC(this%cl_mask))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,15,sizeof(this%cl_rtz1),C_LOC(this%cl_rtz1))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,16,sizeof(this%cl_rtz2),C_LOC(this%cl_rtz2))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,17,sizeof(this%cl_beta),C_LOC(this%cl_beta))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,18,sizeof(this%n),C_LOC(this%n))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,19,sizeof(this%cl_g4),C_LOC(this%cl_b))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,20,sizeof(this%cl_g4),C_LOC(this%cl_gd))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,21,sizeof(this%cl_g4),C_LOC(this%cl_dg))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,22,sizeof(this%cl_g4),C_LOC(this%cl_v))
    if (err.ne.0) stop 'clSetKernelArg'

    err=clSetKernelArg(this%cl_cg_gs_kernel,0,sizeof(this%cl_x),C_LOC(this%cl_x))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,1,sizeof(this%cl_p),C_LOC(this%cl_p))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,2,sizeof(this%cl_res),C_LOC(this%cl_res))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,3,sizeof(this%cl_w),C_LOC(this%cl_w))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,4,sizeof(this%cl_w),C_LOC(this%cl_w_tmp))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,5,sizeof(this%cl_mult),C_LOC(this%cl_mult))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,6,sizeof(this%cl_g1),C_LOC(this%cl_g1))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,7,sizeof(this%cl_g2),C_LOC(this%cl_g2))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,8,sizeof(this%cl_g3),C_LOC(this%cl_g3))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,9,sizeof(this%cl_g4),C_LOC(this%cl_g4))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,10,sizeof(this%cl_g5),C_LOC(this%cl_g5))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,11,sizeof(this%cl_g6),C_LOC(this%cl_g6))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,12,sizeof(this%cl_dx),C_LOC(this%cl_dx))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,13,sizeof(this%cl_dxt),C_LOC(this%cl_dxt))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,14,sizeof(this%cl_mask),C_LOC(this%cl_mask))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,15,sizeof(this%cl_rtz1),C_LOC(this%cl_rtz1))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,16,sizeof(this%cl_rtz2),C_LOC(this%cl_rtz2))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,17,sizeof(this%cl_beta),C_LOC(this%cl_beta))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,18,sizeof(this%n),C_LOC(this%n))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,19,sizeof(this%cl_g4),C_LOC(this%cl_b))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,20,sizeof(this%cl_g4),C_LOC(this%cl_gd))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,21,sizeof(this%cl_g4),C_LOC(this%cl_dg))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,22,sizeof(this%cl_g4),C_LOC(this%cl_v))
    if (err.ne.0) stop 'clSetKernelArg'
  ...
  subroutine cg_fpga_populate(this, r, w, x, Xh,  c_Xh, bclst, gs_Xh, rtz1, beta)
    ...
    err=clSetKernelArg(this%cl_cg_kernel,23,sizeof(this%gs_m),C_LOC(this%gs_m))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,24,sizeof(this%lo),C_LOC(this%lo))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_kernel,25,sizeof(this%nb),C_LOC(this%nb))
    if (err.ne.0) stop 'clSetKernelArg'
    
    err=clSetKernelArg(this%cl_cg_gs_kernel,23,sizeof(this%gs_m),C_LOC(this%gs_m))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,24,sizeof(this%lo),C_LOC(this%lo))
    if (err.ne.0) stop 'clSetKernelArg'
    err=clSetKernelArg(this%cl_cg_gs_kernel,25,sizeof(this%nb),C_LOC(this%nb))
    if (err.ne.0) stop 'clSetKernelArg'
  ...
  function cg_fpga_solve(this, Ax, x, f, n, coef, blst, gs_h, niter) result(ksp_results)
    ...
    do iter = 1, max_iter
       ...
       err=clEnqueueTask(this%cmd_queue,this%cl_cg_gs_kernel,0,C_NULL_PTR,C_NULL_PTR)
       if (err .ne. 0) stop 'clEnqueueEnqueueTask cg'
