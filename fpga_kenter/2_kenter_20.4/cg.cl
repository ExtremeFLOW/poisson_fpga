...
// Cache size is specified in bytes here, thus 64 bytes = 512 bit = 1 cache line
// could possibly instantiate much larger cache if beneficial; probably use 2 different sizes then, smaller cache for idx / gd
#define CACHE_LINES 1024
#define CACHE_S (CACHE_LINES * 64)

// populate w_tmp from previous iteration w
__kernel void cg_main(__global double * restrict x,
                 __global double * restrict p,
                 __global double * restrict r,
                 __global double * restrict w,
                 __global double * restrict w_tmp,
                 ...
                 const int nb)
{
    ...
    for(unsigned ele = 0; ele < N; ele += LX1*LY1*LZ1){
        ...
        #pragma unroll 32
        for(unsigned ijk=0; ijk<LX1*LY1*LZ1; ++ijk)
            w_tmp[ijk + ele] = shw[ijk];
    }
}

// perform gather-scatter from w_tmp into w
__kernel void cg_gs(__global double * restrict x,
                 __global double * restrict p,
                 __global double * restrict r,
                 __global double * restrict w,
                 __global double * restrict w_tmp,
                 ...
                 const int nb)
{
    // copy w_tmp into w; gather-scatter elements will be overwritten afterwards
    #pragma unroll 8
    for(unsigned ele = 0; ele < N; ele ++){
        w[ele] = w_tmp[ele];
    }
    //gather-scatter
    int k = 0;
    ...
    for(int i = 0; i < nb; i++){
        ...
        for(int j = 1; j < blk_len; j++){
            idx[j] = gd[k+j] - 1;
            double cur = rtr_copies3[M2-1] + __burst_coalesced_cached_load(&w_tmp[idx[j]], CACHE_S);
            #pragma unroll
    ...
    for(int i = (o-1); i < m; i+=2){
        double tmp_val;
        double tmp_sum;
        for(int j=0; j<3; j++){ // artificially run for 3 cycles
            int idx = __burst_coalesced_cached_load(&gd[i+(j%2)], CACHE_S) - 1; // index 0, 1, 0
            double val;
            if(j<2){ // first 2 cycles load
                val = __burst_coalesced_cached_load(&w_tmp[idx], CACHE_S);
            }
            // conditional updates
            if(j==0) tmp_val = val;
            if(j==1) tmp_sum = val + tmp_val;
            if(j>0) w[idx] = tmp_sum; // last 2 cycles store
        }
    }
}
