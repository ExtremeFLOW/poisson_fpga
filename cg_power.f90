...
module cg_fpga
  ...
  type, public, extends(ksp_t) :: cg_fpga_t
     ...
     integer(c_intptr_t), allocatable :: platform_ids(:), device_ids(:)
...
contains
  ...
  subroutine cg_fpga_init_device(this,idevice, iplatform, bitstream, lx)
    ...
    print *, 'hmm'
    call create_device_context(iplatform, this%platform_ids,&
                               idevice, this%device_ids, this%context, this%cmd_queue)
    call query_platform_info(this%platform_ids(iplatform))
    call read_file(bitstream,binary,filesize)
    print *, 'hmm2'
    length = filesize
    psource = C_LOC(binary)
    prog = clCreateProgramWithBinary(this%context, 1, C_LOC(this%device_ids(idevice)),&
  ...
  function cg_fpga_solve(this, Ax, x, f, n, coef, blst, gs_h, niter) result(ksp_results)
    ...
    call power(this%platform_ids,this%device_ids)
    err=clFinish(this%cmd_queue)
    ksp_results%res_final = rnorm
